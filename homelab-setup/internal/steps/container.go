...
# User/Group Configuration - CRITICAL FOR SECURITY
# PUID/PGID determine what user containers run as inside the container
# Systemd service runs as root (to access Docker socket)
# Docker daemon runs as root (system service)
# Containers run as THIS unprivileged user (security boundary)
# These should match your service account UID/GID (e.g., dockeruser)
PUID=%s
PGID=%s
TZ=%s
